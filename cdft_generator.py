...
CDFT_DIR  = "run_scripts/cdft"
CDFT_OUT  = "amsoutput/cdft"
...
def find_cdft_output(basename):
    path = os.path.join(CDFT_OUT, f"{basename}.out")
...
def write_cdft_run(out_path, basename, atoms, h_indices):
    with open(out_path, "w") as f:
        ...
        f.write("Engine ADF\n")
        f.write(f"  title {basename}_cdft\n")
...
if __name__ == "__main__":
    os.makedirs(CDFT_DIR, exist_ok=True)
    ...
    for n_step in steps:
        ...
        if find_cdft_output(basename) is not None:
            continue
        ...
        run_path = os.path.join(CDFT_DIR, f"{basename}.run")
        sl_path  = os.path.join(CDFT_DIR, f"{basename}.sl")
        atoms = extract_atoms_block(src)
        write_cdft_run(run_path, basename, atoms, h_indices)
